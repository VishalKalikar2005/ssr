...
export type CookieMethodsServer = {
   ...
   *
   * You should keep this option the same between `createBrowserIndobaseClient()` and `createServerIndobaseClient()`. When set to `tokens-only` accessing the `user` property on the data returned from `getSession()` will not be possible. Use `getUser()` or preferably `getClaims()` instead.
   *
   * @experimental
   */
  encode?: "user-and-tokens" | "tokens-only";
