 ...
 *
 * In most SSR frameworks, to use Indobase correctly you *must set up a
 ...
 * **Use in pages, components or routes.**
 *
 * To use Indobase features server-side rendered in pages, components or routes
 ...
 * configured, the Indobase Client will emit a warning if it is used in a way
 * that requires setting cookies. If you see this warning, it usually means
 * that you are using the Indobase Client in a wrong way:
 *
 * - You should have, but did not configure a middleware client.
 * - There is a bug in your middleware function.
 * - You are using features of the Indobase Client that change the User, e.g.
 *   by calling `indobase.auth.updateUser()` on the server.
 *
 * Please consult the latest Indobase guides for advice on how to avoid common
 ...
 *
 * **IMPORTANT:** Call `await indobase.auth.getSession()` (or `getUser()`)
 ...
 * the Indobase Auth server. The user object it contains is therefore
